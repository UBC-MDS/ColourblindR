...
theme_deutera <- function(colour_type){
   ...
   #' @examples:
   #'
   #' ggplot(mpg, aes(x = hwy, y = cty, fill = as.factor(cyl))) +geom_bar(stat = 'identity') + theme_deutera(colour_type = 'fill')
   #'
   #' ggplot(mpg, aes(x = hwy, y = cty, colour = as.factor(cyl))) + geom_point(stat = 'identity')  +  theme_deutera(colour_type = 'colour')
   #'
   #' ggplot(mpg, aes(hwy, colour = as.factor(cyl))) + geom_density() +  theme_deutera(colour_type = 'colour')
   #'
